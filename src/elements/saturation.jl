...
Because vaporpressure = vaporpressure_ice for almost every element, this could be symbolically simplified by dispatching on water separately but this'll do fine for now
"""
function supersaturation(gas::Element, concentration::Density, T::Temperature; 
    relative_humidity::Float64=0.0, cloud_frac::Float64=1.0, kwargs...)::Real
    α = relative_humidity * (1 - cloud_frac) + cloud_frac
    pvap_liquid = vaporpressure(gas, T; kwargs...)
    return min(supsat_core(gas, concentration, T, pvap_liquid) - α, 0)
end

function supersaturation(ice::Ice, concentration::Density, T::Temperature;
    relative_humidity::Float64=0.0, cloud_frac::Float64=1.0, kwargs...)::Real
    α = relative_humidity * (1 - cloud_frac) + cloud_frac
    pvap_liquid = vaporpressure(gas, T; kwargs...)
